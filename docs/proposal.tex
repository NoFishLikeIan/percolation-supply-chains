...
Now assume that \textit{one} cannot observe the supplier decisions of the red good producers but assigns equal probability to every possible configuration. Given that there are more configurations in which diversification is optimal\footnote{
  Diversification is optimal if \inputTikZ{0.12}{diagrams/conf-idyof.tikz} or, with equal probability, \inputTikZ{0.12}{diagrams/conf-covpref.tikz}, and not optimal if \inputTikZ{0.12}{diagrams/conf-covind.tikz}
...
then we can write $i \in g$ with $g \in \G$ if $i$ produces $g$. Only a random subset of the firms, $\F \subseteq \N$, is functional and hence able to deliver supply intermediate goods. The probability with which a firm is functional depends on an idiosyncratic component and on the choice of its suppliers. 
