...
\begin{equation}
  s_{k + 1}(\mu_k, \rho_k) = \arg\max_{s \in \mathbb{N}_0} \Pi(s) = \arg\max_{s \in \mathbb{N}_0} \left\{ \pi \Big(1 - G_{\mu}(\mu_k, \rho_k, s)\Big) - \kappa \  s \right\}.
\end{equation}
...
\begin{equation}
  \tilde{s}_{k + 1}(\mu_k, \rho_k) \coloneqq \arg\max_{s \in \mathbb{R}_{\geq 0}} \Pi(s)
\end{equation}
...
\begin{equation}
  s_{k + 1} \in \big\{ \ceil{\bar{s}}, \floor{\bar{s} - 1} \big\} \cap \big\{ \ceil{\tilde{s}_{k + 1}}, \floor{\tilde{s}_{k + 1}} \big\}
\end{equation}

This allows us to identify it and study its properties starting from $\tilde{s}$.
