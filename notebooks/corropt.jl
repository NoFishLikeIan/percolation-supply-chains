...
- Number of layers, ``K`` $(@bind K Slider(20:100, show_value = true, default = 45))
...
let
	...
	plot!(ofig, 1:K, ρs; c = :darkred, marker = :o, label = "Firm")
	plot!(ofig, 1:K, ρp; c = :darkblue, marker = :o, label = "Planner")
end
...
md"
## Exploring the first order condition
"
...
begin
	g₁(fₛ, σₛ, s) = G([fₛ, σₛ]; s = s, model) |> first
	g₂(fₛ, σₛ, s) = G([fₛ, σₛ]; s = s, model) |> last
end

# ╔═╡ 752b9212-6c35-4586-8513-6b29d94341aa
begin
	sspace = range(0.1, 4.; length = 101)
	σspace = range(0, m÷2; length = 101)
	fspace = range(0, m; length = 101)
	
	ffig = contourf(
		σspace, sspace, (σₛ, s) -> g₁(fₛ, σₛ, s) - fₛ;
		xlabel = L"\sigma^2_{k-1}", ylabel = L"s",
		title = L"f_{k} - f_{k-1}",
		clims = (-4, 4), c = :coolwarm
	)

	σfig = contourf(
		fspace, sspace, (fₛ, s) -> g₂(fₛ, σₛ, s) - σₛ;
		xlabel = L"f_{k-1}", ylabel = L"s",
		title = L"\sigma^2_{k} - \sigma^2_{k-1}",
		clims = (-4, 4), c = :coolwarm
	)	
