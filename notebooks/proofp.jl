...
function p(s, v)
	if m - s - v < 0 return 1. end
	
	a = gamma(m - s) * gamma(m - v)
	b = gamma(m - s - v) * gamma(m)

	return 1 - a / b
end

# ╔═╡ e343d82b-98fe-4c08-920c-4649f3a4ae8b
function simulatep(s, α, β; N = 100_000)
...
# ╔═╡ 23075649-b778-484f-ad6c-6d037ca415d2
md"

``\alpha``: $(@bind α Slider(0.01:0.01:10, show_value = true, default = 1))

``\beta``: $(@bind β Slider(0.01:0.01:10, show_value = true, default = 1))
...
function simulateNₘ(α, β, k; N = 100_00)
	F = BetaBinomial(m, α, β)
	F̂ = rand(F, N)
